...
export const useGoodsStore = defineStore('goods', () => {
  ...
  function filterList(params: { keyword?: string; category?: string; minPrice?: number; maxPrice?: number }) {
    return onSaleList.value.filter(g => {
      if (params.keyword && !g.title.includes(params.keyword)) return false
      if (params.category && params.category !== '全部' && g.category !== params.category) return false
