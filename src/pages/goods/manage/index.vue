...
<template>
  <view>
    <wd-navbar title="我的发布" safe-area-inset-top fixed placeholder left-arrow @click-left="goBack" />
    ...
    <view style="padding: 12px;">
      ...
      <wd-card v-for="item in list" :key="item.id" :title="item.title" style="margin-bottom: 12px;">
        <view style="display: flex; gap: 12px;">
          <wd-img :src="item.images[0] || 'https://img.yzcdn.cn/vant/defaultpic.png'" width="80" height="80" radius="4" />
          <view style="flex: 1;">
            ...
            <view style="margin-top: 4px;">
              <wd-tag :type="(item.status === 'on_sale' ? 'success' : 'primary') as any" size="small">
        ...
        <view style="margin-top: 12px; display: flex; gap: 8px; justify-content: flex-end;">
          ...
          <wd-button size="small" :type="(item.status === 'on_sale' ? 'warning' : 'success') as any" @click="toggleStatus(item)">
