...
const canBuy = computed(() => userStore.isLogin && !isOwner.value && goods.value?.status === 'on_sale')
...
<template>
  <view v-if="goods">
    <wd-navbar :title="goods.title" safe-area-inset-top fixed placeholder left-arrow @click-left="goBack" />

    <wd-swiper :list="goods.images.length ? goods.images : ['https://img.yzcdn.cn/vant/defaultpic.png']" autoplay height="240px" style="margin-top: 44px;" />
    ...
    <view style="padding: 16px; background: #fff;">
      <view style="display: flex; align-items: baseline; gap: 8px;">
        ...
        <view style="font-size: 13px; color: #999; text-decoration: line-through;">原价 ¥{{ goods.originalPrice }}</view>
      </view>
      ...
      <view style="font-size: 14px; color: #666; margin-top: 12px; line-height: 1.6;">{{ goods.description }}</view>
      <view style="font-size: 12px; color: #999; margin-top: 12px;">浏览 {{ goods.viewCount }} · 发布于 {{ new Date(goods.createdAt).toLocaleDateString() }}</view>
    </view>
    ...
    <view style="position: fixed; bottom: 0; left: 0; right: 0; background: #fff; border-top: 1px solid #eee; padding: 8px 16px; display: flex; gap: 12px; padding-bottom: calc(8px + env(safe-area-inset-bottom));">
      <wd-button size="large" style="flex: 1;" @click="goChat">联系卖家</wd-button>
      <wd-button v-if="canBuy" type="primary" size="large" style="flex: 2;" @click="openBuy">立即购买</wd-button>
      <wd-button v-else-if="isOwner" size="large" style="flex: 2;" disabled>自己发布的商品</wd-button>
      <wd-button v-else size="large" style="flex: 2;" disabled>不可购买</wd-button>
    </view>

    <wd-popup v-model="showBuyPopup" position="bottom" :style="{ padding: '16px', paddingBottom: 'calc(16px + env(safe-area-inset-bottom))' }">
      <view style="font-size: 16px; font-weight: bold; margin-bottom: 12px; text-align: center;">确认购买</view>
