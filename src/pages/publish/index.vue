  ...
  <view>
    ...
    <view style="padding: 16px;">
      ...
      <view style="margin-top: 16px;">
        ...
        <view style="display: flex; flex-wrap: wrap; gap: 8px;">
          <view v-for="(img, idx) in images" :key="idx" style="position: relative;">
            ...
            <wd-icon name="close"
              style="position: absolute; top: -4px; right: -4px; background: #f44; color: #fff; border-radius: 50%; padding: 2px;"
              @click="removeImage(idx)" />
          </view>
          <view
            style="width: 80px; height: 80px; border: 1px dashed #ccc; display: flex; align-items: center; justify-content: center; border-radius: 4px;"
            @click="onUpload">
      ...
      <view style="margin-top: 24px;">
        <wd-button type="primary" block @click="submit">{{ isEdit ? '保存修改' : '立即发布' }}</wd-button>
      </view>
    </view>
  </view>
</template>
