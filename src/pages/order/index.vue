...
<template>
  <view>
    ...
    <view style="padding: 12px;">
      ...
      <wd-card v-for="item in list" :key="item.id" style="margin-bottom: 12px;" @click="goDetail(item.id)">
        <view style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <wd-tag :type="(statusType[item.status] as any)" size="small">{{ statusText[item.status] }}</wd-tag>
          <view style="font-size: 12px; color: #999;">{{ new Date(item.createdAt).toLocaleString() }}</view>
        </view>

        <view style="display: flex; gap: 12px;">
          <wd-img :src="item.goods?.images[0] || 'https://img.yzcdn.cn/vant/defaultpic.png'" width="70" height="70" radius="4" />
          <view style="flex: 1;">
            <view style="font-size: 15px; font-weight: bold;">{{ item.goods?.title }}</view>
            <text style="color: #f44; font-size: 15px; margin-top: 4px;">¥{{ item.goods?.price || 0 }}</text>
          </view>
        </view>
      </wd-card>
    </view>
